...
return [
     ...
     */
    // Raised 10MB -> 1GB so ambassador (UGC) video submissions can be stored.
    // This is the global Spatie ceiling; each feature is still capped by its own
    // form-request rule (UGC video = 1GB, SocialTask proofs = 5MB/img, etc.), so
    // raising it here only lifts the shared upper bound, it doesn't loosen any
    // individual upload. Keep in sync with StoreUgcSubmissionRequest's max: rule.
